...
if len(sys.argv)>=2:
    ...
    print("EX: python show_result.py 05_wiki_RCA_seg0 05_wiki_RCA-aging_seg0 ")
    sys.exit()
# exp_name = "exampleLRU"
...
for exp_name, OHR, segOHR, region in zip(exp_names, OHRs, segOHRs, regions):
    policy_name = exp_name.split('_')[2] 
    plt.plot([region*(x+1) for x in range(len(segOHR))], segOHR, label=policy_name, linewidth=2)  # 藍色預設，label用於圖例
